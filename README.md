...
`防剧透` 和 `antijutou` 是同一个命令，只需要填写数字，默认单位固定为分钟，不需要填写 `min` 或“分钟”。参数支持任意正数、负数和小数：正数增加额外延迟，负数减少额外延迟，小数按分钟计算。

例如，`/hltv 防剧透 20` 表示增加 20 分钟，Rating 总延迟变为 21 分钟；`/hltv antijutou -2` 表示从当前额外延迟中减少 2 分钟；`0.5` 表示半分钟。额外延迟最低为 0，总延迟始终至少为默认的 1 分钟。

设置对所有用户全局生效，但只作用于当前赛事的全部比赛，不会影响其他赛事。为避免改错，同时追踪多个赛事时插件会要求先只保留目标赛事的订阅。命令执行后会直接显示当前额外延迟和 Rating 总延迟。
...
| `/hltv 防剧透 <数字>`、`/hltv antijutou <数字>` | 同一命令；单位默认分钟，正数增加、负数减少，支持小数 |
