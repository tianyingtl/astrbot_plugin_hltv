...
from .core.knowledge import HltvKnowledgeService, match_team_query
...
class HltvPlugin(Star):
    ...
    def _team_query_match(query: str, m: dict) -> bool:
        return match_team_query(query, m)
    ...
    async def query_hltv(
        ...
        Args:
            ...
            query(string): 查询条件，无条件时传空字符串。live/schedule/results 可传战队、赛事名称/简称及天数，如“NAVI 7天”或“EWC”；战队或选手资料传名称；ranking 传 hltv、vrs、asia、europe 或 americas；news 传空字符串、新闻序号或英文关键词；top20 传年份及可选名次，如“2025”或“2025 18”。
        """
        try:
            service = HltvKnowledgeService(
                self.client,
                max_items=self.max_items,
                default_days=self.default_days,
                live_score_loader=self._fetch_live_scores,
            )
            return await service.query(category, query)
        except HltvError as e:
